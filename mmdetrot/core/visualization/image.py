...
from mmdet.core.visualization.image import color_val_matplotlib, EPS
...
def rbbox_4pts(xywhr):
    ...
    dir1 = np.stack((cosr, sinr), axis=-1)
    dir2 = np.stack((-sinr, cosr), axis=-1)
    w /= 2
    h /= 2
...
def imshow_det_rbboxes(img,
    ...
    if labels.shape[0] > 0:
        if mask_color is None:
            ...
            # specify  color
            mask_colors = [np.array(mmcv.color_val(mask_color)[::-1],
                                    dtype=np.uint8)] * (max(labels) + 1)
